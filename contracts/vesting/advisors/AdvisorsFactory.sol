...
contract AdvisorsFactory is Ownable{
    ...
    constructor(address _pushToken, uint256 _start, uint256 _cliffDuration) public {
        require(_pushToken != address(0), "AdvisorsFactory::constructor: pushtoken is the zero address");
        require(_cliffDuration > 0, "AdvisorsFactory::constructor: cliff duration is 0");
        require(_start.add(_cliffDuration) > block.timestamp, "AdvisorsFactory::constructor: cliff time is before current time");
    ...
    function deployAdvisor(address beneficiary, uint256 start, uint256 cliffDuration, uint256 duration, bool revocable, uint256 amount) external onlyOwner returns(bool){
        Advisors advisorContract = new Advisors(beneficiary, start, cliffDuration, duration, revocable);
     ...
     */
    function revokeAdvisorTokens(Advisors advisorVestingAddress) external onlyOwner returns(bool){
    ...
    function withdrawTokens(uint amount) external onlyOwner returns(bool){
        require(block.timestamp > cliff, "AdvisorsFactory::withdrawTokens: cliff period not complete");
        IERC20 pushTokenInstance = IERC20(pushToken);
        uint256 balance = pushTokenInstance.balanceOf(address(this));
        require(amount <= balance, "AdvisorsFactory::withdrawTokens: amount greater than balance");
