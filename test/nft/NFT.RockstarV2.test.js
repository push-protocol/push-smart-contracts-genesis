...
describe("Rockstar Contract test cases", () => {
    ...
    beforeEach(async() => {
        NFT = await ethers.getContractFactory("RockstarV2");
    ...
    describe("safeMint()", function () {
      ...
      it("Should revert if hash is already in use", async function () {
        const tx = await NFTInstance.safeMint(alice.address, 'abc');
        await expect(NFTInstance.safeMint(alice.address, 'abc')).to.be.revertedWith("RockstarV2::safeMint: hash already in use");
      });
    ...
    describe("burn()", function () {
      ...
      it("Should revert if caller is not tokenOwner", async function () {
        ...
        await expect(NFTInstance.connect(bob).burn(tokenId)).to.be.revertedWith("RockstarV2::burn: caller is not owner nor approved");
      });
...
describe('burn()', function () {
  it('reverts when burning a non-existent token id', async function () {
    await expect(NFTInstance.burn(firstTokenId)).to.be.revertedWith("RockstarV2::burn: burn of nonexistent token");
  });
  ...
  context('with minted tokens', function () {
    ...
    context('with burnt token', function () {
      ...
      it('reverts when burning a token id that has been deleted', async function () {
        await NFTInstance.connect(alice).burn(firstTokenId);
        await expect(NFTInstance.connect(alice).burn(firstTokenId)).to.be.revertedWith("RockstarV2::burn: burn of nonexistent token");
      });
    });
  });
});
