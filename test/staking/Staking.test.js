...
describe('Staking', function () {
    ...
    describe('Deposit', function () {
        it('Reverts if amount is <= 0', async function () {
            await expect(
                staking.connect(user).deposit(pushToken.address, 0),
        ...
        it('Saves users deposit in state', async function () {
            ...
            const balance = await staking.balanceOf(userAddr, pushToken.address)
        ...
        it('Updates the pool size of the next epoch', async function () {
            ...
            expect((await staking.getEpochPoolSize(pushToken.address, 1)).toString()).to.be.equal(amount.toString())
        })

        it('Updates the user balance of the next epoch', async function () {
            await pushToken.transfer(userAddr, amount.mul(10))
            await pushToken.connect(user).approve(staking.address, amount.mul(10))

            await staking.connect(user).deposit(pushToken.address, amount)

            expect(
                (await staking.getEpochUserBalance(userAddr, pushToken.address, 1)).toString(),
            ...
            expect(
                (await staking.getEpochUserBalance(userAddr, pushToken.address, 2)).toString(),
        ...
        describe('Continuous deposits', function () {
            beforeEach(async function () {
                await pushToken.transfer(userAddr, amount.mul(10))
                await pushToken.transfer(ownerAddr, amount.mul(10))
                await pushToken.connect(user).approve(staking.address, amount.mul(10))
                await pushToken.connect(owner).approve(staking.address, amount.mul(10))
            })

            it('Deposit at random points inside an epoch sets the correct effective balance', async function () {
                await moveAtEpoch(1)
                await staking.manualEpochInit([pushToken.address], 0)
            ...
            it('deposit epoch 1, deposit epoch 4', async function () {
                ...
                await moveAtEpoch(4)
                await staking.manualEpochInit([pushToken.address], 3)
            ...
            it('deposit epoch 1, deposit epoch 5, deposit epoch 5', async function () {
                ...
                await moveAtEpoch(5)
                await staking.manualEpochInit([pushToken.address], 3)
                await staking.manualEpochInit([pushToken.address], 4)
    ...
    describe('Withdraw', function () {
        it('Reverts if user has no balance', async function () {
            await expect(
                staking.connect(user).withdraw(pushToken.address, amount),
    ...
    describe('Epoch logic', function () {
        ...
        it('deposit in epoch 0, withdraw in epoch 3', async function () {
            ...
            await moveAtEpoch(3)
            await staking.manualEpochInit([pushToken.address], 2)
        ...
        it('deposit in epoch 3, withdraw in epoch 3', async function () {
            await moveAtEpoch(3)
            await staking.manualEpochInit([pushToken.address], 0)
            await staking.manualEpochInit([pushToken.address], 1)
            await staking.manualEpochInit([pushToken.address], 2)
    ...
    describe('Events', function () {
        ...
        it('Deposit emits Deposit event', async function () {
            await expect(staking.connect(user).deposit(pushToken.address, 10))
        ...
        it('Withdraw emits Withdraw event', async function () {
            ...
            await expect(staking.connect(user).withdraw(pushToken.address, 10))
                .to.emit(staking, 'Withdraw')
        })

        it('ManualEpochInit emits ManualEpochInit event', async function () {
            await moveAtEpoch(1)
            await expect(staking.manualEpochInit([pushToken.address], 0))
        ...
        it('EmergencyWithdraw emits EmergencyWithdraw event', async function () {
            ...
            await expect(staking.connect(user).emergencyWithdraw(pushToken.address))
    ...
    async function withdraw (u, x) {
        return await staking.connect(u).withdraw(pushToken.address, x)
    }

    async function getEpochPoolSize (epochId) {
        return (await staking.getEpochPoolSize(pushToken.address, epochId)).toString()
    }

    async function getEpochUserBalance (u, epochId) {
        return (await staking.getEpochUserBalance(u, pushToken.address, epochId)).toString()
    }
