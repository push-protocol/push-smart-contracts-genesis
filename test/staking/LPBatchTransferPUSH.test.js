...
describe("BatchTransferPUSH Contract tests", function () {
  ...
  it("Should revert if someone else tries to send tokens", async function() {
    ...
    const tx = contract.connect(alice).transferPUSH(token.address, individualTransferInfo.recipients, individualTransferInfo.amounts, 0, 101)
  ...
  it("should revert if array of recipient is not equal to array of amounts", async function () {
    ...
    expect(individualTransferInfo.recipients.length).to.be.equal(101)
    expect(individualTransferInfo.amounts.length).to.be.equal(100)

    await expect(contract.transferPUSH(token.address, individualTransferInfo.recipients, individualTransferInfo.amounts, 0, 101))
